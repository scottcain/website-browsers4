...
#	next if ($species =~ /pacificus/ || $species =~ /malayi/);
#	next if ($species =~ /elegans/ || $species =~ /pacificus/ || $species =~ /malayi/ || $species =~ /brenneri/ || $species =~ /remanei/);
...
sub load_gffdb {
	...
	my $cmd = "$Bin/../util/process_elegans_gff.pl $acedb $files | gzip -cf > $gff_archive.new.gz";
...
#    my $load_cmd = "bp_bulk_load_gff.pl --user root --password kentwashere -c -d $db --fasta $fasta_archive $gff_archive.gz 2> /dev/null";
    my $load_cmd = "bp_fast_load_gff.pl --user root --password kentwashere -c -d $db --fasta $fasta_archive $gff_archive.gz 2> /dev/null";
    ...
    if ($species =~ /elegans/) {
        ...
        my $result = system "bp_load_gff.pl -d $db --user root --pass kentwashere --fasta $est </dev/null";
        system("gzip $est");
    }
}
