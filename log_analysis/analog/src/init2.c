...
void configcols(void *opt, char *cmd, char *arg1, char *arg2, int rc) {
  ...
  for (c = arg1; *c != '\0' && i < COL_NUMBER - 1; c++, i++) {
    ...
    if ((cols[i] == COL_DATE && strchr(arg1, 'D') != NULL) ||
...
Choices chartchoices[] = {{"ON", CHART_SORTBY}, {"TRUE", CHART_SORTBY},
  ...
  {"", 0}};
#endif
Choices outstylechoices[] = {{"HTML", HTML}, {"PLAIN", PLAIN},
  {"ASCII", ASCII}, {"LATEX", LATEX}, {"COMPUTER", COMPUTER},
  {"PREFORMATTED", COMPUTER}, {"CRO", COMPUTER}, {"CACHE", OUT_NONE},
  {"NONE", OUT_NONE}, {"", 0}};
#ifndef NODNS
...
Choices langchoices[] = {{"ARMENIAN", ARMENIAN}, {"BOSNIAN", OLDLANG},
  {"BULGARIAN", BULGARIAN}, {"BULGARIAN-MIK", BULGARIAN_MIK},
  {"CATALAN", CATALAN}, {"SIMP-CHINESE", OLDLANG}, {"CHINESE", OLDLANG},
...
void selectlang(char *country, Outchoices *op) {
  /* Select localisation files with country code. If outstyle is HTML
  ...
  if (op->outstyle == HTML || op->outstyle == ASCII) {
    sprintf(partname, "%s%c%clng", country, (op->outstyle == HTML)?'h':'a',
	    EXTSEP);
    filename = buildfilename(LANGDIR, "lang", partname);
  }
  if ((op -> outstyle != HTML && op->outstyle != ASCII) ||
      (f = FOPENR(filename)) == NULL) {
...
#endif   /* NOGLOB */
#else    /* RISCOSDIRENT */
/* The RISC OS dirent is due to Stefan Bellon */
static struct {
