...
static unsigned char gzmagic[2] = {0x1f, 0x8b}; /* gzip magic header */
...
{             /* open for reading */
  ...
  /* If the file is a gz file, we will set lf->gzfile -- otherwise we will set
     lf->file, and lf->ispipe will record whether it's a pipe or not. */
  lf->ispipe = FALSE;
  lf->file = NULL;
  lf->gzfile = NULL;
...
#endif
#ifndef LINE_PARSER   /* The Mac version has its own gzip detection */
  /* If it didn't match an UNCOMPRESS, see if it's a gzipped file from its
     first two bytes, and if so, use zlib. */
  if ((unsigned char)(fgetc(lf->file)) == gzmagic[0] &&
      (unsigned char)(fgetc(lf->file)) == gzmagic[1]) {
    fclose(lf->file);
    lf->file = NULL;
    lf->gzfile = gzopen(lf->name, "rb");
    if (lf->gzfile == NULL) {
...
int my_lfclose(Logfile *p, Logfile *allc, Logfile *allp,
  ...
  *bys7 += p->bytes7;
  ...
  if (p->gzfile != NULL)
    ...
    rc = fclose(p->file);    /* Not much can go wrong with fclose. I hope. */
#ifndef NOPIPES
  else if (feof(p->file)) {
    if ((rc = pclose(p->file)) != 0)
      warn('F', TRUE, "Problems uncompressing %s %s", filetype, p->name);
  }
  else
    rc = pclose(p->file); /* not reached EOF: pclose will return broken pipe */
#endif
  return(rc);
}
...
choice getmoredata(Logfile *lf, char *start, size_t length) {
  ...
  if (lf->gzfile == NULL)   /* normal file or pipe */
    block_end = start + fread((void *)start, 1, length, lf->file);
  else {  /* gz file */
    n = gzread(lf->gzfile, (void *)start, length);
    block_end = start + (size_t)((n < 0)?0:n); /* gzread returns -1 on error */
  }
