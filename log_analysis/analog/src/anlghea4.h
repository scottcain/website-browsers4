...
extern logical do_aliasx(FILE *outf, Outchoices *od, char *name,
  Alias *aliashead);
...
#endif
extern void printalias(FILE *outf, Outchoices *od, char *name, AliasTo *to,
  int pmatch[]);
...
extern int my_lfclose(Logfile *p, Logfile *allc, Logfile *allp, char *filetype,
  Dateman *dman, unsigned long *totdata, double *bys, double *bys7);
