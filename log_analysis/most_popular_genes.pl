...
while (<>) {
    ...
    if (my ($request) = /\"GET\shttp\:\/\/.*\/db\/gene\/gene\?name=(.+?)[;&%\s].*\s?HTTP.*/) {
...
if ($format eq 'TEXT') {
  printf OUT "%-15s %-15s %-15s %-8s %-40s\n",
    qw/WBGeneID Locus MolecularID Total_accesses(Total_accessors)/;
...
#  print OUT join("\t",'#WBGeneID','Locus','MolecularID','Total_accesses (Total accessors)','Accessors (Accesses)'),"\n";
  print OUT join("\t",'#WBGeneID','Locus','MolecularID','Total_accesses (Total accessors)'),"\n";
} else {
