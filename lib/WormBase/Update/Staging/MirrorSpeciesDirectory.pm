package WormBase::Update::Staging::MirrorSpeciesDirectory;
...
has 'step' => (
    is => 'ro',
    default => 'mirror the species directory only for a new release from the Hinxton FTP site',
    );
...
#    } 
    ...
    if ($release =~ /^WS.*/) {
	$local_path  = $self->ftp_releases_dir . "/$release/species";
	$remote_path = $self->remote_ftp_releases_dir . "/$release/species";	
    } else {
...
wget -r -N -nH -l 20 --cut-dirs=5 ftp://ftp.ebi.ac.uk/$remote_path
END
;
