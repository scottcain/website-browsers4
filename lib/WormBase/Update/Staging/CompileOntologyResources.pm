...
has 'search_data_preprocessed_file' =>
  ( is => 'ro', default => 'search_data_preprocessed.txt' );
...
has 'id2association_counts_file' =>
  ( is => 'ro', default => 'id2association_counts.txt' );
has 'id2total_association_file' =>
  ( is => 'ro', default => 'id2total_associations.txt' );
...
sub _build_gene_datadir {
    ...
    my $release = $self->release;
    my $gene_datadir =
      join( "/", $self->support_databases_dir, $release, 'gene' );
    return $gene_datadir;
}
...
sub run {
    ...
    $self->get_cummulative_association_counts(
        $self->id2total_association_file );
...
sub copy_ontology {
    ...
    my $target =
      join( "/", $self->support_databases_dir, $release, 'ontology' );
    $self->system_call( "cp -r $source $target", "copying ontology" );
...
sub compile_search_data {
    ...
    my $release = $self->release;
    my $obo_file_name =
      join( "/", $datadir, sprintf( $self->$obo_tag, $release ) );
    my $association_file_name =
      join( "/", $datadir, sprintf( $self->$assoc_tag, $release ) );

    open HTML, "< $obo_file_name"
      or $self->log->logdie("Cannot open the protein file: $obo_file_name; $!");

    my $target = join( "/", $datadir, $self->search_data_preprocessed_file );
    open OUT, ">>$target"
      or $self->log->logdie("Cannot open the output file $target");
    ...
    foreach my $obo_file_line (<HTML>) {
        ...
        if ( $obo_file_line =~ m/^id\:/ ) {
            ...
            print OUT
"$id\|$term\|$def\|$namespace\|$synonym_list\|$annotation_count\n";
            @synonyms = ();
...
sub compile_ontology_relationships {
    ...
    my $base_output =
      ( $format == 1 ) ? $self->id2parents_file : $self->parent2ids_file;
    my $output = join( "/", $datadir, $base_output );

    open HTML, "< $obo_file_name"
      or $self->log->logdie("Cannot open the protein file: $obo_file_name");
    open OUT, ">>$output"
      or $self->log->logdie("Cannot open the output file: $output $!");
...
sub parse_search_data {
    ...
    my $datafile =
      join( "/", $self->datadir, $self->search_data_preprocessed_file );
    open OUT, ">$output"
      or $self->log->logdie("Cannot open the output file: $output $!");
...
sub clean_up_search_data {
    ...
    open OUT, ">$output"
      or $self->log->logdie("COuldn't open $output for writing");
...
sub get_geneid2go_ids {
    ...
    open BP, ">$datadir/go_bp_id2gene_id.txt"
      or $self->log->logdie("Can't open bp");
    open MF, ">$datadir/go_mf_id2gene_id.txt"
      or $self->log->logdie("Can't open bp");
    open CC, ">$datadir/go_cc_id2gene_id.txt"
      or $self->log->logdie("Can't open bp");
...
sub get_pheno_gene_data_not {
    ...
    open IN_XGENE, "<$indir/gene_xgene_pheno.txt"
      or $self->log->logdie("Can't open in file $indir/gene_xgene_pheno.txt");
    open IN_VAR, "<$indir/variation_data.txt"
      or $self->log->logdie("Can't open in file $indir/variation_data.txt");
    open IN_RNAi, "<$indir/gene_rnai_pheno.txt"
      or $self->log->logdie("Can't open in file $indir/gene_rnai_pheno.txt");
    open OUT, ">$outdir/pheno2gene_names_not.txt"
      or $self->log->logdie(
        "Can't open out file $outdir/pheno2gene_names_not.txt");
...
sub get_pheno_gene_data {
    ...
    open IN_XGENE, "<$indir/gene_xgene_pheno.txt"
      or $self->log->logdie("Can't open xgene in file");
    open IN_VAR, "<$indir/variation_data.txt"
      or $self->log->logdie("Can't open variation in file");
    open IN_RNAi, "<$indir/gene_rnai_pheno.txt"
      or $self->log->logdie("Can't open rnai in file");
    open OUT, ">$outdir/pheno2gene_names.txt"
      or $self->log->logdie("Can't open out file");
...
sub get_pheno_rnai_data {    ## NEEDS UPDATE
    ...
    open IN, "<$indir/gene_rnai_pheno.txt"
      or $self->log->logdie("Can't open infile");
...
sub get_pheno_variation_data {
    ...
    open IN, "<$indir/variation_data.txt"
      or $self->log->logdie("Can't open infile");
    open OUT, ">$outdir/$outfile" or $self->log->logdie("Can't open outfile");
...
sub get_pheno_xgene_data {
    ...
    open IN, "<$indir/gene_xgene_pheno.txt"
      or $self->log->logdie("Can't open in file");
    open OUT, ">$outdir/$outfile" or $self->log->logdie("Can't open outfile");
