...
sub run {
	...
	foreach my $bioproject (@$bioprojects) {
	    ...
	    my $gff2_file = $bioproject->gff_file;       # this includes the full path.
	    
	    next unless ($bioproject->gff_version == 2);
	    $self->log->debug("creating GFF3 file for $name: $id");
	
	    my $symbolic_name = $species->symbolic_name;
	    my $release_dir   = $bioproject->release_dir;
	    my $gff3_file     = join("/",$release_dir,"$symbolic_name.$id.$release.annotations.gff3");       
