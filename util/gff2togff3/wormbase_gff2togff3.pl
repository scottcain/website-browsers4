...
while (my $line = <FILE>) {
    ...
    if ($TRANSCRIPT_INDEX_FEATURES{$key} || $CDS_INDEX_FEATURES{$key}) {
        ...
        foreach my $attribute (qw(Gene Transcript CDS WormPep Brigpep Remapep Jappep Brepep)) {
